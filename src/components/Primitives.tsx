...
const videoFiles = import.meta.glob('/public/videos/*.{mp4,webm}', { eager: true, query: '?url', import: 'default' });
export function ResearchMedia({ name, poster, alt, className = '', eager = false }: { name: string; poster: string; alt: string; className?: string; eager?: boolean }) {
  ...
  const available = Object.keys(videoFiles).find(path => path.endsWith(`/${name}.webm`)) ?? Object.keys(videoFiles).find(path => path.endsWith(`/${name}.mp4`));
  useEffect(() => {
    const video = videoRef.current;
    if (!video) return;
    const preference = window.matchMedia('(prefers-reduced-motion: reduce)');
    let visible = false;
    const update = () => { if(visible && !preference.matches) void video.play().catch(() => {}); else video.pause(); };
  ...
  if (available && !failed) return <video ref={videoRef} className={className} src={asset(available.replace('/public/', ''))} poster={asset(poster)} muted loop playsInline controls aria-label={alt} preload="metadata" onError={() => setFailed(true)} />;
