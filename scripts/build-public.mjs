...
const client = path.join(root, 'tmp/public-client');
...
async function collect(directory) {
  for (const item of await readdir(directory, { withFileTypes: true })) {
    ...
    if (/(^|\/)versions(\/|$)|\.pdf$/i.test(relative)) throw new Error(`Excluded archive entered client build: ${relative}`);
...
// No dist/client or other static output: the Worker serves only the explicit current-design asset set.
await rm(client, { recursive: true, force: true });
await rm(entry);
console.log(`Public Worker built with ${Object.keys(assets).length} V3 assets. No PDFs or old versions packaged.`);
